...
# At 600 points per hour the cut is 8 minutes away, so a quarter is 120s and
# the check must not hold for 100s... it must still hold, so use a faster climb.
...
- Produces: a `trend` shell function.
...
Append to `hooks/clusage-guard.test.sh`, in the guard rail section:
...
git add hooks/clusage-guard.sh hooks/clusage-guard.test.sh
git commit -m "feat(hook): name the trend in a threshold deny"
```
