...
### Task 10: Chain the wake-ups past the one hour cap

**Files:**
- Modify: `hooks/clusage-guard.sh`, the `retry` function
- Test: `hooks/clusage-guard.test.sh`

**Interfaces:**
- Consumes: nothing new.
- Produces: extended `retry` copy on the reset branch only.

Task 6 made the deny copy ask the user before parking. That copy stops at "set a
timer". It does not say how, and the how has two traps a session hit in practice.

A wake-up caps at one hour, so a longer wait needs several. A gap over 55
minutes also lets the prompt cache expire, and the re-send then costs real
money: one observed session paid about 4 dollars to re-send 412k tokens after a
138 minute gap. So the legs must be 55 minutes or less, whatever the total wait.

The second trap is worse. An agent that wakes and calls a tool to check the
clock can have that call denied by this very guard, which is what happened. The
leg count therefore travels inside the wake-up text, so no interim leg needs to
read a clock or touch the filesystem.

Only the reset branch gets this. The no-reset branch has nothing to wait for,
and `stop` covers an exhausted window where overage already pays.

- [ ] **Step 1: Write the failing tests**

Append to `hooks/clusage-guard.test.sh`, beside the Task 6 retry copy cases:
...
Run: `bash hooks/clusage-guard.test.sh`
Expected: FAIL on the four `run` cases. The no-reset case already passes.

- [ ] **Step 3: Extend the reset branch of `retry`**

In `hooks/clusage-guard.sh`, append these six sentences to the string the reset
branch echoes, after the existing "Set a timer or a wake-up only if the user
picks the first option." Keep them in the same single `echo`.

```
If the user waits, chain the wake-ups. A wake-up caps at one hour, and a gap over 55 minutes expires the prompt cache. Use legs of 55 minutes or less. Count them in the wake-up text, as leg N of M. On an interim leg, schedule the next leg and do nothing else. Never call a tool to check the clock, because a tool call can be denied.
```

Change nothing else. Do not touch the no-reset branch and do not touch `stop`.
...
In `README.md`, in the guard rail section, after the paragraph describing what a
deny tells the agent, add:
...
- [ ] **Step 7: Two residual nits from the final re-review**

Both are in `hooks/clusage-guard.sh` and its test, and both are two characters.

First, the `INTERVAL_MIN` guard added by the final fix uses `^[0-9]+$`. That
admits `030`, which awk reads as 30 but bash arithmetic reads as octal 24. It
also admits `08` and `09`, which make the arithmetic print a base error. Tighten
the pattern to `^([1-9][0-9]*|0)$`, or force base ten with `10#`.

Second, in `hooks/clusage-guard.test.sh`, the `run` helper's failure line reads
`$3` unguarded, so a two-argument allow case aborts the whole suite under
`set -u` instead of printing one failure. About ten callers pass two arguments.
Change that one echo to use `${3:-}`.

Add a case proving `CLUSAGE_GUARD_INTERVAL_MIN=030` behaves as 30 and not 24.

- [ ] **Step 8: Commit**

```bash
bash hooks/clusage-guard.test.sh
git add hooks/clusage-guard.sh hooks/clusage-guard.test.sh README.md
git commit -m "feat(hook): chain the wait into legs that keep the cache warm"
```
